  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={DialerNumbers}
        renderItem={({ item }) => (
            <TouchableOpacity onPress={()=>setPhoneNumber(prev=>prev+item.text)}
            onLongPress={()=>item.text==='0'?setPhoneNumber(prev=>prev+'+'):null}
            >
      ...
      <TouchableOpacity  style={styles.backSpace}>
           

        </TouchableOpacity>
        <TouchableOpacity style={styles.dialerButton} onPress={()=>{
        ...
        <TouchableOpacity onPress={() => setPhoneNumber((prev) => prev.slice(0, -1))} style={styles.backSpace}>
            {phoneNumber && <FontAwesome5 name="backspace" size={calcHeight(3)} color='black' />}
